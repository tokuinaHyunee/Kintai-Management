...
public class AttendanceService {
    ...
    public List<AttendanceRecordDto> getList(Employee employee, YearMonth ym) {
        LocalDate start = ym.atDay(1);
        LocalDate end   = ym.atEndOfMonth();

        Map<LocalDate, AttendanceRecordDto> byDate = new LinkedHashMap<>();
        for (WorkTime wt : workTimeRepository
                .findWithGoOutRecordsByEmployeeAndDateBetween(employee.getEmployeeId(), start, end)) {
            byDate.put(wt.getWorkDate(), AttendanceRecordDto.from(wt, wt.getGoOutRecords()));
        }
    ...
    public void clockIn(Employee employee) {
        ...
        if (wt != null && wt.getStartTime() != null) {
            throw new BusinessException("本日はすでに出勤打刻済みです");
        }
        if (wt == null) {
            wt = new WorkTime();
    ...
    public void clockOut(Employee employee) {
        ...
        if (wt.getStartTime() == null) {
            throw new BusinessException("出勤打刻がありません");
        }
    ...
    @Transactional
    public void goOut(Employee employee) {
        LocalDate today = LocalDate.now();
        WorkTime wt = workTimeRepository
                .findByEmployeeEmployeeIdAndWorkDate(employee.getEmployeeId(), today)
                .orElseThrow(() -> new BusinessException("出勤打刻がありません"));

        if (wt.getStartTime() == null) throw new BusinessException("出勤打刻がありません");
        if (wt.getEndTime() != null)   throw new BusinessException("退勤済みです");
        if (goOutRecordRepository
                .findTopByWorkTimeWorkIdAndReturnTimeIsNullOrderByOutTimeDesc(wt.getWorkId())
                .isPresent()) {
            throw new BusinessException("すでに外出中です");
        }
    ...
    @Transactional
    public void goOutReturn(Employee employee) {
        LocalDate today = LocalDate.now();
        WorkTime wt = workTimeRepository
                .findByEmployeeEmployeeIdAndWorkDate(employee.getEmployeeId(), today)
                .orElseThrow(() -> new BusinessException("出勤打刻がありません"));
        ...
        GoOutRecord active = goOutRecordRepository
                .findTopByWorkTimeWorkIdAndReturnTimeIsNullOrderByOutTimeDesc(wt.getWorkId())
                .orElseThrow(() -> new BusinessException("外出打刻がありません"));
    ...
    @Transactional
    public void saveMemo(Employee employee, String workDate, String memo) {
        WorkTime wt = workTimeRepository
                .findByEmployeeEmployeeIdAndWorkDate(
                        employee.getEmployeeId(), LocalDate.parse(workDate))
                .orElseThrow(() -> new BusinessException("勤務記録がありません"));
        wt.setWorkMemo(memo);
        workTimeRepository.save(wt);
    }

    @Transactional
    public int submitCsv(Employee employee, String fileName, String csvContent) {
        ...
        submission.setFileName(fileName != null ? fileName : "submission.csv");
    ...
    @Transactional
    public Map<String, Object> importLegacy(Employee employee, List<Map<String, String>> records) {
        ...
        return Map.of("successCount", success, "errorCount", error, "errors", List.of());
    }
}
